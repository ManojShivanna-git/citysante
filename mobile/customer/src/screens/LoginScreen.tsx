...
export default function LoginScreen() {
  ...
  const handleSendOTP = async () => {
    ...
    if (cleaned.length !== 10) { Alert.alert('Invalid number', 'Enter a valid 10-digit mobile number'); return }
    setLoading(true)
    try {
  ...
  const handleVerifyOTP = async () => {
    ...
    setLoading(true)
    try {
      const res = await authApi.verifyOTP(phone.replace(/\D/g, ''), otpStr, isNewUser ? name || undefined : undefined)
      const { user, accessToken, refreshToken, isNewUser: newUser } = res.data.data
      if (newUser && !user.name?.trim()) {
        // Need name
        setIsNewUser(true)
        setStep('name')
        // Store tokens temporarily
        await SecureStore.setItemAsync('customer_refresh_token', refreshToken)
    ...
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
          ...
          {step === 'otp' && (
            <>
              <TouchableOpacity onPress={() => setStep('phone')} style={styles.backBtn}>
                ...
                <Text style={[styles.subtitle, { color: RED, marginLeft: 4 }]}>Change number</Text>
              </TouchableOpacity>
        ...
        <Text style={styles.terms}>
          ...
          <Text style={{ color: RED }}>Terms of Service</Text> and{' '}
...
const styles = StyleSheet.create({
  ...
  nameInput: {
    ...
    fontSize: 16, color: '#111', backgroundColor: '#fafafa', marginBottom: 20,
  },
